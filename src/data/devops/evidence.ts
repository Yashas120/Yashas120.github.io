...
export const evidence: EvidenceFact[] = [
  ...
  professional({
    ...
    outcome: "Overall deployment time fell by approximately 50% without bypassing prerequisite gates.",
    status: "shipped",
  ...
  professional({
    ...
    outcome: "Approximately four hours of routine manual generation and publication per SDK fell to zero while version choice remained human-controlled.",
    status: "shipped",
  ...
  professional({
    ...
    outcome: "Page-load time improved by approximately 40%, with less unnecessary memory work and a shorter user-facing path.",
    status: "shipped",
  ...
  professional({
    ...
    claim: "Worked on AWS infrastructure where SNS sent notifications to SQS queues in both regions before regional service consumers applied database updates.",
    ...
    contribution: "Worked on the Terraform and integration boundaries between the data source, SNS notifications to both regional SQS queues, consumers, and database dependencies.",
  ...
  },
  {
    ...
    claim: "Rebuilt and configured Linux for ghOSt, then compared kernel-only and user-space scheduling policies through controlled RocksDB runs.",
    shortClaim: "Repeatable ghOSt scheduler experiment",
    problem: "Scheduler behavior on RocksDB had to be compared under controlled load, thread, and memory conditions.",
    contribution: "Built the ghOSt-enabled kernel environment and analyzed CFS, FIFO, and user-space scheduling configurations.",
